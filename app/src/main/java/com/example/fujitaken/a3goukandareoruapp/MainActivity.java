...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null){
            ...
            //OruFragment.javaのnewInstanceメソッドを呼んで画面を生成(厳密にはreplaceなので置き換え)
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {   //引数itemにbottom_navigation_menuで選択されたものが入る
                switch (item.getItemId()){  //switch文で処理分岐 それぞれreplaceでfragmentを置き換え
                    ...
                    case R.id.menu_main:
                        OruFragment oruFragment = OruFragment.newInstance();
                        getSupportFragmentManager().beginTransaction().replace(R.id.container, oruFragment).commit();
                        break;

                    case R.id.menu_state:
                        OranFragment oranFragment = OranFragment.newInstance();
                        getSupportFragmentManager().beginTransaction().replace(R.id.container, oranFragment).commit();
                        break;
                }
